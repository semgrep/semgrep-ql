(**
   Boilerplate to be used as a template when mapping the ql CST
...
let map_aggid (env : env) (x : CST.aggid) =
  ...
  | `Concat tok -> R.Case ("Concat",
      (* "concat" *) token env tok
    )
  | `Stri_18c266c tok -> R.Case ("Stri_18c266c",
      (* "strictconcat" *) token env tok
    )
  | `Count tok -> R.Case ("Count",
      (* "count" *) token env tok
    )
  | `Max tok -> R.Case ("Max",
      (* "max" *) token env tok
  ...
  | `Rank tok -> R.Case ("Rank",
      (* "rank" *) token env tok
    )
  | `Stri_8bc2381 tok -> R.Case ("Stri_8bc2381",
      (* "strictcount" *) token env tok
    )
  | `Stri_a43e15b tok -> R.Case ("Stri_a43e15b",
      (* "strictsum" *) token env tok
    )
  | `Sum tok -> R.Case ("Sum",
      (* "sum" *) token env tok
    )
...
let map_upper_id (env : env) (x : CST.upper_id) =
  (match x with
  | `Pat_3bf1220 x -> R.Case ("Pat_3bf1220",
      map_pat_3bf1220 env x
    )
  | `Semg_meta tok -> R.Case ("Semg_meta",
      (* semgrep_metavariable *) token env tok
    )
...
let map_classname (env : env) (x : CST.classname) =
  map_upper_id env x
...
let map_varname (env : env) (x : CST.varname) =
  map_simpleid env x

let map_modulename (env : env) (x : CST.modulename) =
  map_simpleid env x
...
let map_variable (env : env) (x : CST.variable) =
  ...
  | `Result tok -> R.Case ("Result",
      (* "result" *) token env tok
    )
  | `Varn x -> R.Case ("Varn",
      map_varname env x
    )
...
let rec map_aritylesspredicateexpr (env : env) ((v1, v2) : CST.aritylesspredicateexpr) =
  let v1 =
    (match v1 with
...
and map_moduleexpr (env : env) (x : CST.moduleexpr) =
  ...
  | `Modu x -> R.Case ("Modu",
      map_moduleinstantiation env x
    )
  | `Modu_COLONCOLON_choice_simp (v1, v2, v3) -> R.Case ("Modu_COLONCOLON_choice_simp",
      let v1 = map_moduleexpr env v1 in
      let v2 = (* "::" *) token env v2 in
      let v3 =
        (match v3 with
...
and map_moduleinstantiation (env : env) ((v1, v2, v3, v4, v5) : CST.moduleinstantiation) =
...
and map_signatureexpr (env : env) (x : CST.signatureexpr) =
  ...
  | `Modu x -> R.Case ("Modu",
      map_moduleexpr env x
    )
  | `Pred x -> R.Case ("Pred",
      map_predicateexpr env x
    )
  )

and map_typeexpr (env : env) (x : CST.typeexpr) =
  (match x with
  | `Opt_modu_COLONCOLON_clas (v1, v2) -> R.Case ("Opt_modu_COLONCOLON_clas",
      let v1 =
        (match v1 with
        | Some (v1, v2) -> R.Option (Some (
            let v1 = map_moduleexpr env v1 in
  ...
  | `Dbtype tok -> R.Case ("Dbtype",
      (* pattern @[a-z][A-Za-z0-9_]* *) token env tok
    )
  | `Prim x -> R.Case ("Prim",
      map_primitivetype env x
    )
  )
...
let map_anon_choice_pred_44fe1b2 (env : env) (x : CST.anon_choice_pred_44fe1b2) =
  (match x with
  | `Pred tok -> R.Case ("Pred",
      (* "predicate" *) token env tok
    )
  | `Type x -> R.Case ("Type",
      map_typeexpr env x
    )
...
let map_annotation (env : env) (x : CST.annotation) =
  ...
  | `Anno_LBRACK_anno_rep_COMMA_anno_RBRACK (v1, v2, v3, v4, v5) -> R.Case ("Anno_LBRACK_anno_rep_COMMA_anno_RBRACK",
...
let map_anon_vard_rep_COMMA_vard_76ab5f3 (env : env) ((v1, v2) : CST.anon_vard_rep_COMMA_vard_76ab5f3) =
  let v1 = map_vardecl env v1 in
  let v2 =
    R.List (List.map (fun (v1, v2) ->
      let v1 = (* "," *) token env v1 in
...
let rec map_anon_call_arg_rep_COMMA_call_arg_25882ee (env : env) ((v1, v2) : CST.anon_call_arg_rep_COMMA_call_arg_25882ee) =
...
and map_call_or_unqual_agg_body (env : env) (x : CST.call_or_unqual_agg_body) =
  (match x with
  | `Call_body (v1, v2, v3) -> R.Case ("Call_body",
      let v1 = (* "(" *) token env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
            map_anon_call_arg_rep_COMMA_call_arg_25882ee env x
          ))
        | None -> R.Option None)
      in
      let v3 = (* ")" *) token env v3 in
  ...
  | `Unqual_agg_body (v1, v2, v3, v4, v5, v6) -> R.Case ("Unqual_agg_body",
      let v1 = (* "(" *) token env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
            map_anon_vard_rep_COMMA_vard_76ab5f3 env x
          ))
        | None -> R.Option None)
      in
      let v3 = (* "|" *) token env v3 in
...
and map_exprorterm (env : env) (x : CST.exprorterm) =
  (match x with
  | `Spec_call (v1, v2, v3) -> R.Case ("Spec_call",
      let v1 = (* "none" *) token env v1 in
      let v2 = (* "(" *) token env v2 in
      let v3 = (* ")" *) token env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Prefix_cast (v1, v2, v3, v4) -> R.Case ("Prefix_cast",
      let v1 = (* "(" *) token env v1 in
      let v2 = map_typeexpr env v2 in
      let v3 = (* ")" *) token env v3 in
  ...
  | `Conj (v1, v2, v3) -> R.Case ("Conj",
      let v1 = map_exprorterm env v1 in
      let v2 = (* "and" *) token env v2 in
      let v3 = map_exprorterm env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Disj (v1, v2, v3) -> R.Case ("Disj",
      let v1 = map_exprorterm env v1 in
      let v2 = (* "or" *) token env v2 in
      let v3 = map_exprorterm env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Impl (v1, v2, v3) -> R.Case ("Impl",
      let v1 = map_exprorterm env v1 in
      let v2 = (* "implies" *) token env v2 in
      let v3 = map_exprorterm env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Quan (v1, v2, v3, v4) -> R.Case ("Quan",
      let v1 = map_quantifier env v1 in
      let v2 = (* "(" *) token env v2 in
      let v3 =
        (match v3 with
        | `Opt_vard_rep_COMMA_vard_opt_BAR_expr_opt_BAR_expr (v1, v2) -> R.Case ("Opt_vard_rep_COMMA_vard_opt_BAR_expr_opt_BAR_expr",
            let v1 =
              (match v1 with
              | Some x -> R.Option (Some (
                  map_anon_vard_rep_COMMA_vard_76ab5f3 env x
...
and map_full_aggregate_body (env : env) (x : CST.full_aggregate_body) =
  (match x with
  | `Opt_vard_rep_COMMA_vard_BAR_opt_expr_opt_BAR_asexprs_opt_orders (v1, v2, v3, v4) -> R.Case ("Opt_vard_rep_COMMA_vard_BAR_opt_expr_opt_BAR_asexprs_opt_orders",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_anon_vard_rep_COMMA_vard_76ab5f3 env x
  ...
  | `Vard_rep_COMMA_vard x -> R.Case ("Vard_rep_COMMA_vard",
      map_anon_vard_rep_COMMA_vard_76ab5f3 env x
    )
  )
...
and map_par_expr (env : env) (x : CST.par_expr) =
  (match x with
  | `LPAR_simp_semg_meta_RPAR (v1, v2, v3, v4) -> R.Case ("LPAR_simp_semg_meta_RPAR",
      let v1 = (* "(" *) token env v1 in
      let v2 = map_modulename env v2 in
...
and map_primary (env : env) (x : CST.primary) =
  ...
  | `Qual_expr (v1, v2, v3) -> R.Case ("Qual_expr",
  ...
  | `Super_ref (v1, v2) -> R.Case ("Super_ref",
      let v1 =
        (match v1 with
        | Some (v1, v2) -> R.Option (Some (
            let v1 = map_typeexpr env v1 in
  ...
  | `Aggr (v1, v2, v3, v4, v5) -> R.Case ("Aggr",
      ...
      let v4 =
        ...
        | Some x -> R.Option (Some (
            (match x with
            | `Full_aggr_body x -> R.Case ("Full_aggr_body",
                map_full_aggregate_body env x
              )
            | `Expr_aggr_body x -> R.Case ("Expr_aggr_body",
                map_expr_aggregate_body env x
              )
            )
          ))
        | None -> R.Option None)
  ...
  | `Range (v1, v2, v3, v4, v5) -> R.Case ("Range",
      let v1 = (* "[" *) token env v1 in
      let v2 = map_exprorterm env v2 in
      let v3 = (* ".." *) token env v3 in
      let v4 = map_exprorterm env v4 in
  ...
  | `Set_lit (v1, v2, v3, v4) -> R.Case ("Set_lit",
      let v1 = (* "[" *) token env v1 in
      let v2 =
        (match v2 with
        | Some (v1, v2) -> R.Option (Some (
  ...
  | `Par_expr x -> R.Case ("Par_expr",
      map_par_expr env x
    )
  | `Expr_anno (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Expr_anno",
      let v1 = map_annotname env v1 in
      let v2 = (* "[" *) token env v2 in
...
and map_qualifiedrhs (env : env) (x : CST.qualifiedrhs) =
  (match x with
  | `Pred_opt_clos_LPAR_opt_call_arg_rep_COMMA_call_arg_RPAR (v1, v2, v3, v4, v5) -> R.Case ("Pred_opt_clos_LPAR_opt_call_arg_rep_COMMA_call_arg_RPAR",
...
let map_optbody (env : env) (x : CST.optbody) =
  (match x with
  | `Empty tok -> R.Case ("Empty",
      (* ";" *) token env tok
    )
  | `Body x -> R.Case ("Body",
      map_body env x
    )
  | `High (v1, v2, v3, v4, v5, v6, v7, v8) -> R.Case ("High",
      let v1 = (* "=" *) token env v1 in
      let v2 = map_literalid env v2 in
      let v3 = (* "(" *) token env v3 in
      let v4 =
...
let map_datatypebranch (env : env) ((v1, v2, v3, v4, v5, v6, v7) : CST.datatypebranch) =
  let v1 =
    (match v1 with
    | Some tok -> R.Option (Some (
        (* pattern \/\*\*[^*]*\*+([^/*][^*]*\*+)*\/ *) token env tok
...
let map_memberpredicate (env : env) ((v1, v2, v3, v4, v5, v6) : CST.memberpredicate) =
...
let map_classlesspredicate (env : env) ((v1, v2, v3) : CST.classlesspredicate) =
  ...
  let v3 =
    (match v3 with
    | `LPAR_opt_vard_rep_COMMA_vard_RPAR_optb (v1, v2, v3, v4) -> R.Case ("LPAR_opt_vard_rep_COMMA_vard_RPAR_optb",
        let v1 = (* "(" *) token env v1 in
        let v2 =
          (match v2 with
          | Some x -> R.Option (Some (
...
let map_classmember (env : env) (x : CST.classmember) =
  ...
  | `Rep_anno_choice_char (v1, v2) -> R.Case ("Rep_anno_choice_char",
...
let map_dataclass (env : env) ((v1, v2, v3) : CST.dataclass) =
  ...
  let v3 =
    (match v3 with
    | `Opt_extends_type_rep_COMMA_type_opt_inst_type_rep_COMMA_type_choice_LCURL_rep_clas_RCURL (v1, v2, v3) -> R.Case ("Opt_extends_type_rep_COMMA_type_opt_inst_type_rep_COMMA_type_choice_LCURL_rep_clas_RCURL",
        let v1 =
          (match v1 with
          | Some (v1, v2, v3) -> R.Option (Some (
        ...
        let v3 =
          (match v3 with
          | `LCURL_rep_clas_RCURL (v1, v2, v3) -> R.Case ("LCURL_rep_clas_RCURL",
              let v1 = (* "{" *) token env v1 in
    ...
    | `Type_e6aca0c x -> R.Case ("Type_e6aca0c",
        map_typealiasbody env x
      )
    | `Type_9cc1977 x -> R.Case ("Type_9cc1977",
        map_typeunionbody env x
      )
    )
  in
  R.Tuple [v1; v2; v3]
...
let rec map_module_ (env : env) ((v1, v2, v3, v4, v5) : CST.module_) =
  ...
  let v5 =
    (match v5 with
    | `LCURL_rep_modu_RCURL (v1, v2, v3) -> R.Case ("LCURL_rep_modu_RCURL",
        let v1 = (* "{" *) token env v1 in
...
and map_modulemember (env : env) (x : CST.modulemember) =
  ...
  | `Rep_anno_choice_impo (v1, v2) -> R.Case ("Rep_anno_choice_impo",
      let v1 = R.List (List.map (map_annotation env) v1) in
      let v2 =
        (match v2 with
        | `Impo x -> R.Case ("Impo",
            map_importdirective env x
          )
        | `Clas x -> R.Case ("Clas",
            map_classlesspredicate env x
          )
        | `Data_cb44f91 x -> R.Case ("Data_cb44f91",
            map_dataclass env x
          )
        | `Data_3931108 x -> R.Case ("Data_3931108",
            map_datatype env x
          )
...
let map_ql (env : env) (x : CST.ql) =
  ...
  | `Rep_modu xs -> R.Case ("Rep_modu",
      R.List (List.map (map_modulemember env) xs)
    )
  )
